...
\begin{tikzpicture}[
  ...
  phaselabel/.style={font=\footnotesize, text=black!50, align=left},
]
...
\node[skillgold] (ideamine) at (0, 0) {/idea-mine};
\node[desc, below=2pt of ideamine] {PDF $\to$ idea.md};

\node[skill] (paperinit) at (0, -2.2) {/paper-init};
...
%% ============================================================
\node[phase] at (4.5, -9.6) {Cross-cutting Utilities};

\node[skillgray] (resume) at (0, -10.6) {/resume};
\node[desc, below=2pt of resume] {context loader};

\node[skillgray] (figure) at (3, -10.6) {/figure};
\node[desc, below=2pt of figure] {TikZ + Python/R\\Eagle integration};

\node[skillgray] (latextable) at (6, -10.6) {/latex-table};
\node[desc, below=2pt of latextable] {table formatting\\auto-invoke};

\node[skillgray] (webaccess) at (9, -10.6) {/web-access};
...
% Phase 1 -> Phase 2
\draw[arr] (paperinit) -- node[above, desc] {idea.md} (litplan);
...
\node[desc] at (2.6, -0.6) {\textit{methodology.md}};
...
\begin{scope}[on background layer]
  % Phase 1
  \node[fit=(ideamine)(paperinit), inner sep=14pt, fill=bggold!40, rounded corners=6pt] {};
  % Phase 2
