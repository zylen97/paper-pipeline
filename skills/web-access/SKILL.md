...
- **专用工具优先于通用网络工具**：目标平台有成熟的专用工具（CLI 如 `yt-dlp` 抓视频字幕、`gh` 操作 GitHub；MCP server；浏览器/系统插件；官方 API 等）时，先用专用工具——更可靠、更精确，且不消耗 CDP 这种重资源。下表是专用工具不可用、不存在或不支持目标场景时的通用降级链。具体平台的专用工具偏好记录在 `references/site-patterns/{domain}.md`。
