...
对标同方法的已发表论文，建立行业基线，再基于基线审计当前论文的方法论问题并提出结构优化建议。

**核心原则**：
1. **对标先于审计**：先看行业怎么做，再评判我们做得够不够。没有对标数据的审计是盲审。
2. **具体性**：每条质疑必须指向 md 中的具体位置，且不能"换论文仍成立"。
3. **结构优化是必做项**：从对标论文中学习 Methodology/Results 的组织方式。

**输入** `$ARGUMENTS`：可选，指定审计范围。示例：
- `/method-audit` — 全流程（对标 + 审计 + 结构优化）
...
```
1. 检查缓存（最高优先级）：
   IF data/method-benchmark/cross_comparison.md 存在且时间戳 < 7 天:
     → AskUserQuestion：
       "检测到已有对标分析（{date}，{N} 篇论文）。
       (1) 复用现有对标数据（推荐）
       (2) 重新分析"
     用户选 (1) → 跳过 2.1-2.4，直接读取 cross_comparison.md，进入 2.5
     用户选 (2) → 继续检查下一条件
...
确认论文列表？可增删。确认后请将 PDF 放入 data/method-benchmark/
```
...
用户确认列表后，将 PDF 放入 `data/method-benchmark/`（如目录不存在则提示创建）。

Agent 用 Glob 检测 `data/method-benchmark/*.pdf`，模糊匹配文件名。
...
## A. 方法论处理
1. 基本信息（论文标题、期刊、年份、研究情境、软件版本）
2. 数据与样本（节点数/样本量、时期数、时间窗口长度）
3. 模型设定（效应/变量列表、嵌套模型、估计方法和参数）
4. 模型拟合/GOF（有无报告、统计量、结果、处理方式）
5. 稳健性检验（数量、具体项目）
6. 统计报告规范（效应量exp(β)/odds ratio、显著性标准、收敛诊断、边缘显著处理）
7. 缺失数据处理
8. 与本文的对比启示

## B. 论文结构
9. Methodology 章节组织（所有大标题→小标题层级，每节大致篇幅占比）
10. Results 章节组织（报告顺序、表格组织方式、图的使用）
11. 特色做法（值得借鉴的组织方式、表格设计、图表使用）

{METHOD_TYPE 差异化补充——见下方}

报告用中文写，技术术语保留英文。
写入: data/method-benchmark/{citation_key}_benchmark.md
```
...
所有 subagent 完成后，自动生成 `data/method-benchmark/cross_comparison.md`：
...
```markdown
| 论文 | Methodology 小节结构 | Results 小节结构 | 特色做法 |
|:-----|:--------------------|:----------------|:---------|
| **本文** | {当前结构} | {当前结构} | |
| {key1} | {结构} | {结构} | {特色} |
| ... |
...
完整比对表已保存至 data/method-benchmark/cross_comparison.md
```
...
**核心价值所在。** 基于对标数据 + Claude 方法论知识 + 论文具体内容，提出方法论问题和结构优化建议。
...
基于步骤 2 的论文结构比对（表 3），提出 Methodology/Results 的组织优化建议：
...
> Benchmark: {M} 篇对标论文 | 数据: data/method-benchmark/cross_comparison.md
...
### 对标分析配置
- **工作目录**：`data/method-benchmark/`
- **subagent prompt 模板**按 `{METHOD_TYPE}` 差异化（见步骤 2.3）
