...
**两遍扫描机制（解决前置依赖 ID 竞态）**：
- 第一遍：全量生成 MF/ST/PM/SO 条目到 `drafts/_audit_draft.md`（落盘，不只是心算），每条字段齐备但 `前置依赖: <TBD>` 占位，分配好 ID（MF-1, MF-2, ..., ST-1, ..., PM-1, ..., SO-S-1, ...）
- 第二遍：读取落盘的 draft，逐条 Edit 把 `<TBD>` 替换为实际依赖 ID 列表或 `[]`。依赖字段完整后 `mv drafts/_audit_draft.md {目标路径}`。
- **落盘好处**：长上下文下心算两遍易漏填；落盘后第二遍是纯机械替换，可逐条 verify。
...
**前置依赖**: [ST-1, MF-0] | []          # 若依赖其他条目的修复结果，列出 ID；无则填空数组
...
> **前置依赖字段规约**：
> - 用途：步骤 4 队列构建时按拓扑序排序（依赖项必须排在被依赖项之后），防止严重度排序倒挂修复链
> - 空值：`[]`（无依赖，按严重度正常排序）
> - 循环检测：如果拓扑排序发现环（A→B→A），报错并降级为严重度排序，提示用户手动拆环
> - **未知 ID 容错**：若引用了不存在的 ID（手工编辑误写、或该条目已删），**忽略该边并打印警告**，不触发 KeyError；DAG 构建继续进行
> - 跨类型引用合法：MF 可以依赖 ST，ST 可以依赖 PM，互相引用 OK
> - **孤立节点**（无前置依赖、无人依赖它）按严重度插入到同级组内，与 `[]` 节点共享同一层
...
> **依赖拓扑约束（防止严重度排序覆盖修复链）**：
> 每条审计项（ST/MF/SO/PM）若依赖其他条目的修复结果（如 "MF-2 的参数重标定依赖 ST-1 变量定义修正后才能执行"），应在条目的 `前置依赖` 字段声明——**该字段定义见步骤 3.4 的 MF/SO-S/SO-M/SO-F 模板**（两遍扫描机制：第一遍生成 + 分配 ID，第二遍回填依赖）。
> 队列构建时做拓扑排序：若 `A` 的前置依赖 `B` 在队列中，则 `A` 必须排在 `B` 之后，即使 `A` 严重度为 🔴、`B` 为 🟡。纯严重度排序可能把修复链倒挂，导致依赖 B 的修复结果被 A 覆盖或失效。
> 实施：读取每条 `前置依赖` 字段，构建 DAG → 拓扑排序 → 组间按严重度 → 输出队列。
> 循环依赖检测：若发现环（A→B→A），记录警告并降级为严重度排序，提示用户手动拆环。
> **未知 ID 容错**：若某条 `前置依赖` 列表包含 DAG 中不存在的 ID（手工编辑误写、或被依赖条目已删除），忽略该条边并打印警告（`⚠️ MF-3 引用了不存在的 ST-9 — 忽略`），不触发 KeyError。
> **孤立节点**按严重度插入到同级组内。
> 若所有条目 `前置依赖` 字段均为 `[]` 或缺失，等价于按严重度排序（兼容无依赖的小规模审计）。
