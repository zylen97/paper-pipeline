...
### 0.3.5 既有 RIS 清点与命名空间冲突检查

执行 `ls {LIT_DIR}*.ris` 列出既有 RIS 文件，对每个文件提取编号前缀（如 `1-`、`5-`）与主题关键词。

设计完方向（步骤 1）后，主 agent 必须自检 **方向编号 ↔ 既有 RIS 编号的语义一致性**：
- 任何"既有 RIS 的编号前缀被新方向占用，但既有 RIS 主题与新方向主题不一致"的情况，必须在 `literature_search_plan.md` 输出前**显式提示用户**：旧文件归档到 `_archive/` 还是重命名给新方向。
- 检查通过后才能进入步骤 2 配额计算。

**典型冲突场景**：v1.0 plan 的 D5 = "媒介选择"（产出 `5-媒介选择.ris`），v2.X plan 的 D5 改为 "项目组织视角"——若不处理，编号 5 的 RIS 主题与新方向不匹配，会让用户误判 "D5 的 RIS 已就绪 / 待跑"。
