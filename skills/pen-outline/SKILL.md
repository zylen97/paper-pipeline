---
description: "构建叙述型章节的句子级要点（每句话 + citation form），支持 full（逐子节交互）/ quick（仅关键节点交互）双模式，为 /pen-draft 提供高质量输入"
---

# Outline Workflow — 构建章节要点
...
**双模式**（两种模式并行支持，通过 `mode=` 参数切换；默认 full，保留原有完整交互体验）：

| 模式 | 调用 | 交互次数 | 保留的交互点 |
|:-----|:-----|:---------|:-------------|
| **full**（默认） | `/pen-outline section=XXX` 或 `/pen-outline section=XXX mode=full` | 每子节 2 轮 + 若干全局 | 全部 13 个 |
| **quick** | `/pen-outline section=XXX mode=quick` | 全章 3-4 轮 | 关键节点 7 个（section 识别、边界、LR 方向、字数、写入确认） |

> **quick 模式保留**：section 缺失/匹配失败、MD headings 与模板不一致、LR 方向选择、字数 TBD、字数分配表、写入前确认（原子闸门）。
> **quick 模式省略**：LR 写作蓝图、调度开始前、已有要点处理、Intent 逐节确认、要点逐节确认、定位表文献/维度确认——全部自动生成并采用，记入 `issues_log`，在步骤 7 写入前确认时集中展示，由用户一次性 review。

**输入** `$ARGUMENTS`：`section=XXX [mode=full|quick]`（section 必须；mode 可选，默认 full）。
...
- 提取 `section` 参数
- 无 `section` → AskUserQuestion 询问要为哪个 section 构建要点（此交互在两种模式下都保留）
- 提取 `mode` 参数：识别 `mode=quick` 或 `mode=full`；**未指定或值不是 `quick` → 默认 `{MODE}` = `"full"`**（保持旧行为）
- 设置全局变量 `{MODE}` ∈ {`"full"`, `"quick"`}
- 初始化 `{QUICK_ISSUES_LOG}` = `[]`（仅 quick 模式使用，累积所有自动决定供步骤 7/8 展示）

在对话中显示当前模式：
```
🎛 运行模式: {MODE}
  - full: 每子节 intent + 要点逐一确认（默认）
  - quick: 仅关键节点交互（LR 方向 / 字数 / 写入），其他自动生成
```

> **向前兼容性保证**：所有 `{MODE} == "full"` 分支保留原行为，本次改造不改动 full 模式的任何执行逻辑；所有 `{MODE} == "quick"` 分支为新增行为。
...
先在对话中展示差异对比：
```
⚠️ 章节 md 的 ### headings 与模板不一致（可能被之前的 session 误改，也可能是历史编码差异）：

当前 md headings:           模板期望:
──────────────────         ──────────────────
{逐行对比}

Gap 段数量若与 idea.md RQ 数量不一致，会同步适配。
```

然后 **AskUserQuestion**（此交互在两种模式下都保留，破坏性修改必须人工把关）：
```
(1) 恢复为模板结构（推荐；已有内容按语义最近匹配迁移到对应 heading 下）
(2) 只做 Gap 数量适配，保留现有 headings 的标点/编号
(3) 不改结构，继续使用当前 headings（显示警告）
```

用户选 (1) → 恢复 `###` headings 为模板结构 + Gap 数量适配。
用户选 (2) → 仅做 Gap 数量适配（新增/删除 Gap 段），其他 headings 保留当前形态。
用户选 (3) → 维持现状，后续流程按当前 headings 继续；在步骤 7 写入前摘要中以 ⚠️ 标注"未规范化"。
...
**`{MODE}` 分支**：
- `full` → AskUserQuestion 等待用户确认。用户可以：
  - 调整子主题的顺序、增删子主题
  - 更换某个子主题的 citation 分配
  - 修改引用形式（形式A↔形式B）
  - 调整"指出不足"的措辞
  - **循环**：用户不满意 → 修改写作蓝图 → 再次展示 → 直到用户确认
- `quick` → **跳过确认，直接采用自动生成的蓝图**。追加记录到 `{QUICK_ISSUES_LOG}`：
  ```
  [blueprint] 方向 {N} 的写作蓝图已自动生成（子主题 {M} 个、citation 预分配 {K} 个），未经用户确认。
  用户可在步骤 7 写入前确认时查看汇总，或在写入后手动修改 md。
  ```
...
- `quick` → **跳过确认，直接展示计划并开始循环**：
  ...
  逐子节自动生成 intent + 要点，不逐个确认。全部完成后在步骤 7 写入前集中 review。
  ```
...
- **已有要点**（以 `- ` 开头的条目，且包含实质内容而非 TODO）→ 显示已有要点：
  ```
  📋 该子节已有以下要点：
  {列出已有要点}
  ```
  ...
  **`{MODE}` 分支**：
  ...
  - `quick` → **默认走 (3) 重新构建**（幂等性优先，避免旧要点遗留造成的混乱）。追加记录到 `{QUICK_ISSUES_LOG}`：
    ```
    [overwrite] 子节 "{subsection}" 的原有要点已被覆盖（原有 {N} 条）。
    若需保留原有要点，请在步骤 7 写入前取消、切换 full 模式重跑。
    ```
...
**`{MODE}` 分支**：
- `full` → AskUserQuestion：确认 intent 或提出修改意见。
  **循环**：用户不满意 → 修改 intent → 再次展示 → 直到用户确认。
- `quick` → **直接采用自动生成的 intent**，不询问。追加记录到 `{QUICK_ISSUES_LOG}`：
  ```
  [intent] 子节 "{subsection}" 的 intent 已自动生成并采用: "{intent文本}"
  ```
...
- **引用密度目标**：标记 `[citep]` 或 `[citet]` 的要点占总要点数 **≥ 85%**（子节级校验；部分子节整体由合法 [—] 构成，见下方"合法低密度子节白名单"——这些子节豁免 85% 阈值）。仅以下句子类型允许标 `[—]`：
  - 段落间过渡句（如"然而，上述研究存在以下不足"）
  - 本文贡献/创新声明（如"本文通过...填补了这一空白"）
  - RQ/假设陈述（如"据此，本文提出 RQ1：..."）
  - 章节结构引导句（如"本文余下部分结构如下"）
  - **本文方法论选择/设计决策声明**（如"本文保留灰色预测框架并扩展为方向性双通道"、"本文采用 $b^+ \geq 0$ 的符号约束承担方向性"）——这类句子在"方法论概述"子节普遍出现，是对本文方法的事实陈述而非文献归纳，无需引用
...
- **句子分类参考表**（生成要点前，在内部按此表分类每句话的引用需求，不输出给用户）：
  ...
  | 过渡/衔接句 | `[—]` | ~5-8% |
  | 本文论点/贡献/RQ 声明 | `[—]` | ~5-8% |
  | 本文方法论选择/设计决策声明 | `[—]` | 仅白名单子节 ~30-60%，其他子节 0-5% |
  | 结构引导句 | `[—]` | ~2% |
...
**合法低密度子节白名单**（标题模糊匹配，大小写 + 空格不敏感）：

这些子节的功能性内容本身就以本文论点/RQ/贡献/方法声明/结构引导为主，整章引用偏少是预期情况。命中白名单的子节**豁免 85% 阈值**，自检时仅显示 ℹ️ 中性说明，**不触发自动修复**。

| `{SECTION_TYPE}` | 白名单子节（匹配关键词） |
|:---|:---|
| `intro` | "方法论"/"methodology overview"/"RQ"（如"方法论概述与RQ"）；"贡献"/"contribution"；"论文结构"/"paper organization"/"structure" |
| `litrev` | 无（LR 各方向子节都应 ≥85%） |
| `discussion` | "理论含义"/"theoretical implications"；"实践含义"/"practical implications"；"局限"/"limitations"（其中本文论点密度允许偏低） |
...
- 当前子节命中白名单 → **豁免阈值**，显示 `ℹ️ 白名单子节（{RQ/贡献/方法论/...}），低密度属预期；仅列出 [—] 要点供 review，不自动改引用`
- 当前子节**不在**白名单：
  - `density` ≥ 85% → ✅ 通过
  - 80% ≤ `density` < 85% → ⚠️ 略低，建议检查 `[—]` 标记的要点是否确实无需引用
  - `density` < 80% → ❌ 不达标，自动触发修复：
    1. 列出所有 `[—]` 标记的要点
    2. 逐条判断：该句是否真的属于合法 [—] 类型（过渡句 / 本文论点 / 贡献 / RQ / 方法声明 / 结构引导）？如果实际是事实陈述或文献观点，从 `{AVAILABLE_REFS}` 匹配合适的 citation 并改标为 `[citep]` 或 `[citet]`
    3. 修复后重新统计并展示修复后的要点列表

**`{MODE}` 分支**（引用密度自检在两种模式下均自动执行；此处仅差异在"是否等待用户确认"）：
- `full` → AskUserQuestion：确认要点或提出修改意见（可以要求增删改某个要点、换引用、调整引用形式标记、调整段落分组等）。
  **循环**：用户不满意 → 修改要点和引用理由表 → 再次展示（含重新自检） → 直到用户确认。
- `quick` → **直接采用自动生成的要点**，不询问。追加记录到 `{QUICK_ISSUES_LOG}`：
  ```
  [points] 子节 "{subsection}": 自动生成 {N} 条要点（[citep]×{x}, [citet]×{y}, [—]×{z}, (ref)×{r}），引用密度 {density}%
  ```
  若密度 < 80% 触发的自动修复**仍然执行**（非交互修复），修复后结果直接采用。
...
**`{MODE}` 分支**：
- `full` → AskUserQuestion 等待用户确认。**循环**：用户不满意 → 修改文献/维度 → 再次展示 → 直到用户确认。
- `quick` → **直接采用候选文献和默认维度**。追加记录到 `{QUICK_ISSUES_LOG}`：
  ```
  [positioning] 定位表已自动选定 {N} 篇文献 + {K} 个维度（{维度列表}）。
  若需调整，在步骤 7 写入前取消，或在写入后手动修改 md。
  ```

**d. 生成定位表骨架**：用户确认（full）/ 自动确认（quick）后，生成完整的 markdown 定位表暂存（含每篇文献在各维度的填充内容）。bib 文件验证所有 citation key。
...
- **读到 TBD 或无法解析** → 先查询 benchmark 基线作为参考：
  1. 读取 `structure/3_methodology/benchmark/cross_comparison.md` 的"章节字数基线"表（如存在），提取对应 `{SECTION_TYPE}` 的中位数和范围
  2. 如 benchmark 文件不存在 → 使用内置默认值（与 /method-end 对齐）：
     - `intro` → 1100 words
     - `litrev` → 1500 words
     - `discussion` → 1500 words

  然后 **AskUserQuestion**（两种模式下都保留）：
  ```
  ⚠️ {md文件名} 的目标字数尚未确定（仍为 TBD）。
  Benchmark 参考: 中位数 {N} words, 范围 {min}-{max}（或内置默认 {fallback} words）

  (1) 使用 benchmark 默认值 {N} words (Recommended)
  (2) 手动指定字数（输入数字）
  (3) 退出，先运行 /method-audit
  ```
  用户选 (1) → 使用 benchmark 默认值
  用户选 (2) → 使用用户指定值
  用户选 (3) → 退出
...
**quick 模式附加**：当 `{MODE}` == `"quick"` 时，在摘要之后**额外展示** `{QUICK_ISSUES_LOG}` 的完整内容，让用户在写入前集中 review 所有自动决定：
```
⚠️ quick 模式下自动决定的项（共 {K} 条，请集中 review）:
  [blueprint] 方向 1 的写作蓝图已自动生成（子主题 4 个、citation 预分配 12 个）
  [intent] 子节 "3.2 Theoretical foundations" 的 intent 已自动生成并采用: "..."
  [points] 子节 "3.2 ...": 自动生成 8 条要点（[citep]×5, [citet]×2, [—]×1, (ref)×0），引用密度 87%
  [overwrite] 子节 "3.3 ..." 的原有要点已被覆盖（原有 6 条）
  [positioning] 定位表已自动选定 6 篇文献 + 6 个维度（Method / Context / ...）
  ...

如不满意,拒绝写入后切换 full 模式重跑整章，或写入后手动编辑 md。
```

统一展示"确认写入？"的 AskUserQuestion（两种模式都保留此交互）。
...
- **仅 `{MODE}` == `"quick"` 时额外展示**：
  ```
  🤖 quick 模式自动决定摘要（{K} 条，详见步骤 7 写入前的完整清单）:
     - 写作蓝图自动生成: {N1} 次
     - Intent 自动采用: {N2} 次
     - 要点自动采用: {N3} 次
     - 定位表自动选定: {N4} 次
  ...
  若需精修某子节，可切换 full 模式针对性重跑：
    /pen-outline section={parent}/{child} mode=full
  ```
