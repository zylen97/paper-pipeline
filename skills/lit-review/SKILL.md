...
所有 subAgent（含拆分后的）按**槽位制**启动（同时不超过 8 个，完成一个补一个）。
...
**格式强制约束**：核心、重要、备选三个分级的文献列表**必须**使用完整的6列表格（`| # | 作者 | 标题 | 年份 | 期刊 | 入选理由 |`）。禁止以下压缩写法：
- 省略"标题"列（如只写作者/年份/期刊/关键词的4列表格）
- 用段落文字代替表格（如"包括：XXX（Author YYYY）、YYY（Author YYYY）…"）
- 将多篇文献合并为一行（如"8-13 含XXX等"）
...
4. 生成该方向的完整 direction report，**所有分级（核心/重要/备选）必须使用完整6列表格**（`| # | 作者 | 标题 | 年份 | 期刊 | 入选理由 |`）。合并过程中不得将表格压缩为摘要段落或省略标题列。

> **格式校验**：合并后的 report 中，每篇入选文献必须有独立的表格行且包含完整标题。主Agent在写入文件前须自检：report 中的入选文献行数 = 报头声明的入选总数。不一致则修正。
...
4. **清理前验证**：清理前先统计原RIS条目数，清理后统计保留条目数。如果保留数为0，说明匹配逻辑有误，**不得覆写**，改为报告错误

> **注意**：
> - RIS文件按 `TY  -` 开头分割条目，每条到 `ER  -` 结束
> - 读取RIS文件时须使用 `utf-8-sig` 编码（处理可能存在的UTF-8 BOM头），否则首条条目的 `TY  -` 可能因BOM前缀而无法识别
> - 匹配依据为 `TI  -`（标题）字段（主），`AU  -`（第一作者姓氏）+ `PY  -`（年份）字段（辅）。先尝试标题匹配，未命中再尝试作者+年份匹配
